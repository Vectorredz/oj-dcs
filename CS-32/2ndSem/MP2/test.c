...
void PQ_INSERT (PriorityQueue *P, BinaryTreeNode *x)
{
    if (P->size == P->capacity) {
        P->capacity = P->capacity == 0 ? 1 : P->capacity * 2; 
        BinaryTreeNode **new_array = (BinaryTreeNode **) realloc(P->array, P->capacity * sizeof(BinaryTreeNode *));
        if (new_array == NULL) {
            PQ_OVERFLOW(); 
        }
        P->array = new_array;
    ...
    while (i > 0) {
        int j = (i - 1) / 2;
        if (KEY(P, i) < KEY(P, j) ||
            (KEY(P, i) == KEY(P, j) && P->array[i]->DATA < P->array[j]->DATA)) {
...
void HEAPIFY(PriorityQueue *P, int r)
{
    ...
    if (left < P->size) {
       if (P->array[left]->FREQ < P->array[smallest]->FREQ ||
    (P->array[left]->FREQ == P->array[smallest]->FREQ &&
     P->array[left]->DATA < P->array[smallest]->DATA))

            smallest = left;
    }

    if (right < P->size) {
       if (P->array[right]->FREQ < P->array[smallest]->FREQ ||
    (P->array[right]->FREQ == P->array[smallest]->FREQ &&
     P->array[right]->DATA < P->array[smallest]->DATA))
...
BinaryTreeNode *BUILD_HUFFMAN_TREE (PriorityQueue *P)
{
    ...
    while (P->size > 1){
        ...
        z->DATA = (x->DATA < y->DATA) ? x->DATA : y->DATA;
